...
export const Dashboard = () => {
  ...
  const [showUploadResume, setShowUploadResume]= useState(false);{/*upload resume popup window hidden by default*/}
  ...
  const deleteResume= async(resumeID)=>{
    const confirm= window.confirm("Are you shure you want to delete this resume ?")
    if(confirm){
          ...
          showCreateResume && (
            ...
            bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center"> {/*closes create resume popup window*/}
              ...
              rounded-lg w-full max-w-sm p-6">
                ...
                <input type="text" onChange={(e)=> setTitle(e.target.value)} value={title} placeholder='Enter resume title' 
                className="w-full px-4 py-2 mt-2 mb-4 border border-gray-300 
        ...
        {/*Upload Resume popup window*/}
        {
          showUploadResume && (
            ...
            bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center">
              <div onClick={(e)=> e.stopPropagation()} className="relative bg-slate-50 border shadow-md 
              rounded-lg w-full max-w-sm p-6">
                <h2 className="text-center text-xl font-bold  md-4">Upload resume</h2>
                <input type="text" onChange={(e)=> setTitle(e.target.value)} value={title} placeholder='Enter resume title' 
                className="w-full px-4 py-2 mt-2 mb-4 border border-gray-300 
                rounded-lg focus:ring-green-500 focus:border-green-500" required />
                <div>
        ...
        }
        {
          editResumeID && (
            ...
            bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center"> 
              <div onClick={(e)=> e.stopPropagation()} className="relative bg-slate-50 border shadow-md 
              rounded-lg w-full max-w-sm p-6">
                <h2 className="text-center text-xl font-bold  md-4">Edit resume title</h2>
                <input type="text" onChange={(e)=> setTitle(e.target.value)} value={title} placeholder='Enter resume title' 
                className="w-full px-4 py-2 mt-2 mb-4 border border-gray-300 
                rounded-lg focus:ring-blue-500 focus:border-blue-500" required />
                <button className="w-full py-2 bg-green-600 text-white rounded 
